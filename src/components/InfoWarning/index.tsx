...
export const InfoWarning = ({ text, type }: InfoWarningProps): JSX.Element => {
  const { colors } = useTheme();

  const setIcon = (type: InfoWarningType): JSX.Element => {
    switch (type) {
      case 'INFO':
        return <PiInfoBold color={colors.blue} size={22} />;
      default:
        return <PiWarningBold color={colors.yellow} size={22} />;
    }
  };
