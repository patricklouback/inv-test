...
export function ActivityTimeGraph(): JSX.Element {
  ...
  const [accessTimeByMonthState, setAccessTimeByMonth] = useState<
    IAccessTimeByMonth[]
  >([
    {
      date: '00/0000',
  ...
  useEffect(() => {
    async function fetchData(): Promise<void> {
      ...
      const { activeUsers } = await countMembers();

      setTotalsMembers(activeUsers);
    }
    fetchData();
  ...
  const data: ChartOptions<any> = useMemo(() => {
    const savedata = {
      ...
      datasets: [
        {
          label: 'Horas',
          data: accessTimeByMonthState.map(aT =>
            aT.accessTotalHours.toFixed(0)
          ),
  ...
  const averageTime = (): number => {
    ...
    const isValidHour = roundedResult <= 0 ? 0.5 : roundedResult;
