...
export function IdeasPerRouteGraph({ campaignIds }: GlobalIndicatorsProps): JSX.Element {
    ...
    const data: ChartOptions<any> = useMemo(() => {
        const savedata = {
            labels: ideasPerRoute.map(ipr => `${ipr.routeName}: ${total !== 0 ? (Number(ipr.ideasAmount) / total * 100).toFixed(1) : 0}%`),
            datasets: [
                {
